...
public class MongoDBWriter extends Writer {
    ...
    public static class Task extends Writer.Task {
        ...
        public void prepare() {
            super.prepare();
            //获取presql配置，并执行
            String preSql = writerSliceConfig.getString(MongoKeyConstant.PRE_SQL);
...
//              or  { "$or" : [ { "age" : { "$gt" : 27}} , { "age" : { "$lt" : 15}}]}
        ...
        private void doBatchInsert(MongoCollection<BasicDBObject> collection, List<Record> writerBuffer, JSONArray columnMeta) {
            ...
            for (Record record : writerBuffer) {
                ...
                for (int i = 0; i < record.getColumnNumber(); i++) {
                    ...
                    String type = columnMeta.getJSONObject(i).getString(MongoKeyConstant.COLUMN_TYPE);
                    //空记录处理
                    if (Strings.isNullOrEmpty(record.getColumn(i).asString())) {
                        if (MongoKeyConstant.isArrayType(type.toLowerCase())) {
                            data.put(columnMeta.getJSONObject(i).getString(MongoKeyConstant.COLUMN_NAME), new Object[0]);
                        } else {
                    ...
                    if (Column.Type.INT.name().equalsIgnoreCase(type)) {
                        ...
                        try {
                            if (MongoKeyConstant.isObjectIdType(type.toLowerCase())) {
                            ...
                            } else if (MongoKeyConstant.isArrayType(type.toLowerCase())) {
                                String splitter = columnMeta.getJSONObject(i).getString(MongoKeyConstant.COLUMN_SPLITTER);
                                if (Strings.isNullOrEmpty(splitter)) {
                                    throw DataXException.asDataXException(MongoDBWriterErrorCode.ILLEGAL_VALUE,
                                            MongoDBWriterErrorCode.ILLEGAL_VALUE.getDescription());
                                }
                                String itemType = columnMeta.getJSONObject(i).getString(MongoKeyConstant.ITEM_TYPE);
                                if (itemType != null && !itemType.isEmpty()) {
                                    ...
                                    if (itemType.equalsIgnoreCase(Column.Type.DOUBLE.name())) {
                                        ...
                                        data.put(columnMeta.getJSONObject(i).getString(MongoKeyConstant.COLUMN_NAME), list.toArray(new Double[0]));
                                    } else if (itemType.equalsIgnoreCase(Column.Type.INT.name())) {
                                        ArrayList<Integer> list = new ArrayList<Integer>();
                                        for (String s : item) {
                                            list.add(Integer.parseInt(s));
                                        }
                                        data.put(columnMeta.getJSONObject(i).getString(MongoKeyConstant.COLUMN_NAME), list.toArray(new Integer[0]));
                                    } else if (itemType.equalsIgnoreCase(Column.Type.LONG.name())) {
                                        ArrayList<Long> list = new ArrayList<Long>();
                                        for (String s : item) {
                                            list.add(Long.parseLong(s));
                                        }
                                        data.put(columnMeta.getJSONObject(i).getString(MongoKeyConstant.COLUMN_NAME), list.toArray(new Long[0]));
                                    } else if (itemType.equalsIgnoreCase(Column.Type.BOOL.name())) {
                                        ArrayList<Boolean> list = new ArrayList<Boolean>();
                                        for (String s : item) {
                                            list.add(Boolean.parseBoolean(s));
                                        }
                                        data.put(columnMeta.getJSONObject(i).getString(MongoKeyConstant.COLUMN_NAME), list.toArray(new Boolean[0]));
                                    } else if (itemType.equalsIgnoreCase(Column.Type.BYTES.name())) {
                                        ArrayList<Byte> list = new ArrayList<Byte>();
                                        for (String s : item) {
                                            list.add(Byte.parseByte(s));
                                        }
                                        data.put(columnMeta.getJSONObject(i).getString(MongoKeyConstant.COLUMN_NAME), list.toArray(new Byte[0]));
                                    } else {
                                        data.put(columnMeta.getJSONObject(i).getString(MongoKeyConstant.COLUMN_NAME), record.getColumn(i).asString().split(splitter));
                                    }
                                } else {
                                    data.put(columnMeta.getJSONObject(i).getString(MongoKeyConstant.COLUMN_NAME), record.getColumn(i).asString().split(splitter));
                                }
                                ...
                                data.put(columnMeta.getJSONObject(i).getString(MongoKeyConstant.COLUMN_NAME), JSON.toJSON(mode));
                            } else {
                                data.put(columnMeta.getJSONObject(i).getString(MongoKeyConstant.COLUMN_NAME), record.getColumn(i).asString());
                            }
                        } catch (Exception e) {
                        ...
                        if (Column.Type.LONG.name().equalsIgnoreCase(type)) {
                            data.put(columnMeta.getJSONObject(i).getString(MongoKeyConstant.COLUMN_NAME), record.getColumn(i).asLong());
                        } else {
            ...
            if (this.writeMode != null &&
                    this.writeMode.getString(MongoKeyConstant.IS_REPLACE) != null &&
                    MongoKeyConstant.isValueTrue(this.writeMode.getString(MongoKeyConstant.IS_REPLACE))) {
                String uniqueKey = this.writeMode.getString(MongoKeyConstant.UNIQUE_KEY);
        ...
        public void init() {
            ...
            this.userName = writerSliceConfig.getString(MongoKeyConstant.MONGO_USER_NAME);
            this.password = writerSliceConfig.getString(MongoKeyConstant.MONGO_USER_PASSWORD);
            this.database = writerSliceConfig.getString(MongoKeyConstant.MONGO_DB_NAME);
...
//            }
            ...
            try {
                IDataSourceFactoryGetter dsGetter = IDataSourceFactoryGetter.getWriterDataSourceFactoryGetter(this.writerSliceConfig, this.containerContext);
            ...
            this.collection = writerSliceConfig.getString(MongoKeyConstant.MONGO_COLLECTION_NAME);
            this.batchSize = BATCH_SIZE;
            this.mongodbColumnMeta = JSON.parseArray(writerSliceConfig.getString(MongoKeyConstant.MONGO_COLUMN));
            this.writeMode = JSON.parseObject(writerSliceConfig.getString(MongoKeyConstant.WRITE_MODE));
        }
