 ...
 * 但它并不做实际的数据同步操作
 */
public class JobContainer extends AbstractContainer implements IJobContainerContext {
    ...
    private Reader.Job preCheckReaderInit(JobPluginCollector jobPluginCollector) {
        ...
        jobReader.setContainerContext(this);
        // 设置reader的jobConfig
    ...
    private Writer.Job preCheckWriterInit(JobPluginCollector jobPluginCollector) {
        ...
        jobWriter.setContainerContext(this);
        // 设置writer的jobConfig
    ...
    private void adjustChannelNumber() {
        ...
        if (isByteLimit) {
            ...
            Long channelLimitedByteSpeed =
                    this.configuration.getLong(CoreConstant.DATAX_CORE_TRANSPORT_CHANNEL_SPEED_BYTE);
            if (channelLimitedByteSpeed == null || channelLimitedByteSpeed <= 0) {
                throw DataXException.asDataXException(FrameworkErrorCode.CONFIG_ERROR, "在有总bps限速条件下，单个channel的bps" +
                        "值不能为空，也不能为非正数");
            }
        ...
        if (isRecordLimit) {
            ...
            Long channelLimitedRecordSpeed =
                    this.configuration.getLong(CoreConstant.DATAX_CORE_TRANSPORT_CHANNEL_SPEED_RECORD);
            if (channelLimitedRecordSpeed == null || channelLimitedRecordSpeed <= 0) {
                throw DataXException.asDataXException(FrameworkErrorCode.CONFIG_ERROR, "在有总tps限速条件下，单个channel的tps" +
                        "值不能为空，也不能为非正数");
            }
    ...
    private void logStatistics() {
        ...
        long recordSpeedPerSecond =
                communication.getLongCounter(CommunicationTool.READ_SUCCEED_RECORDS) / transferCosts;
    ...
    private List<Configuration> distributeTasksToTaskGroup(int averTaskPerChannel, int channelNumber,
        ...
        for (int i = 0; i < taskGroupNumber; i++) {
            ...
            List<Configuration> taskGroupJobContent =
                    taskGroupConfig.getListConfiguration(CoreConstant.DATAX_JOB_CONTENT);
